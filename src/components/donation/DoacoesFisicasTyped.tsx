...
import { useDoacoesFisicasTyped } from '@/hooks/useDoacoesFisicasTyped';
import FormularioTyped from './FormularioTyped';
import { useAuth } from '@/context/AuthContext';
import { DadosDoacaoForm } from '@/types/doacaoForm';

const DoacoesFisicasTyped: React.FC = () => {
  const { isAuthenticated } = useAuth();
  const { criarDoacao, isSubmitting } = useDoacoesFisicasTyped();
  const [showForm, setShowForm] = useState<boolean>(false);

  const handleSubmit = async (dadosDoacao: DadosDoacaoForm): Promise<boolean> => {
...
export default DoacoesFisicasTyped;
