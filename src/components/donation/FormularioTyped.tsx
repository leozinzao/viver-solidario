...
import { DadosDoacaoForm } from '@/types/doacaoForm';

interface FormularioTypedProps {
  onSubmit: (dados: DadosDoacaoForm) => Promise<boolean>;
...
const FormularioTyped: React.FC<FormularioTypedProps> = ({
  onSubmit,
  isSubmitting,
  onCancel
}) => {
  ...
  const [formData, setFormData] = useState<DadosDoacaoForm>({
    titulo: '',
    descricao: '',
    categoria_id: '',
    quantidade: 1,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (sucesso) {
      // Reset form com tipagem correta
      setFormData({
        titulo: '',
        descricao: '',
        categoria_id: '',
  ...
  const handleTipoEntregaChange = (value: string) => {
    const tipoEntrega = value as 'retirada' | 'entrega_doador';
    setFormData({ 
      ...formData, 
      tipo_entrega: tipoEntrega,
      endereco_entrega: tipoEntrega === 'entrega_doador' ? ENDERECO_ONG : ''
    });
  };

  const handleUnidadeChange = (value: string) => {
    const unidade = value as DadosDoacaoForm['unidade'];
...
export default FormularioTyped;
